...
// import helper
import { orderCost, feeOpenAndClose } from '../../core/helper/bybit/usdtPerpetual'
...
export default function Futures(props) {
  ...
  function loadSocket() {
    ...
    if (userJWT != null && userJWT != "") {
      ...
      socketConnection.on("createResponseUSDTPerpetual", function (data) {
        ...
        //   console.log("createOrderFutures createResponseFutures data : ", data);
        //   if (createdOrder == 1) {
        //     createdOrder = 0;
        //     setSocketOrdCreLoad(false);
        //     if (data.status == 1) {
    ...
    socketConnection.on("USDTPerpetualOrderBook", function (data) {
      // console.log("USDTPerpetualOrderBook", data, data.pairName, pairData.pair.replace('_', ''));
      if (pairData && pairData.pair && pairData.pair.replace('_', '') == data.pairName) {
  ...
  function submitTrade() {
    ...
    // console.log('createOrderFutures : ', { data });
  ...
  function percentageChange(orderType, percentage) {
    let orderValue = JSON.parse(JSON.stringify(orderData));
    const leverageVal = (orderValue.leverage && orderValue.leverage[0]) ? parseInt(orderValue.leverage[0]) : 1;
    ...
    if (userTradeDetails.toBalance > 0) {
      ...
      if (orderData && orderData.price && orderData.amount && orderData.leverage && pairDetails.takerFee && orderData.type) {
        orderData.amount =
          roundValues(
            (
  ...
  function calculateValues(placeValue, placeType, orderDataRestore = {}, extData = {}) {
    // console.log({ placeValue, placeType });
    const {
      priceChange = false
    } = extData;
    let orderValue = JSON.parse(JSON.stringify(orderData));
    ...
    // if(orderValue.price !== orderPrice) {
    ...
    if (orderData && orderData.price && orderData.amount && orderData.leverage && pairDetails.takerFee && orderData.type) {
      ...
      serOrderData((prev) => {
        return {
          ...
          'orderValue': parseFloat(orderData.price) * parseFloat(orderData.amount)
        }
      })
    }
                                ...
                                viewOrderList={
                                  pairDetails
                                    ...
                                    (orderListType === "all" || orderListType === "sell")
                                      ?
                                      sellOB ? sellOB.slice(0, orderListType === "all" ? 9 : 25).reverse() : []
                                      :
                                      buyOB ? buyOB : []
                                    :
                                    []
                            ...
                            {orderListType == "all" &&
                              <div className="d-flex flex-row orderbook-table-heading-section">
                                {pairDetails ? <div className="ps-2">
                                  <span className={`trade-text-5 ${(pairDetails.lastPrice) > 0 ? ((pairDetails.lastPrice) < (pairDetails.marketPrice) ? 'color-green' : 'color-red') : ''}`}>
                                    {decimalValue((pairDetails.marketPrice) ? pairDetails.marketPrice : 0, pairDetails.priceDecimal)}
                                  </span>
                                </div> : ""}
                              </div>
                            }
